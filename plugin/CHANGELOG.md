...
### Added
- Groovy pack: ship the `groovy-language-server` declaration for `.groovy` (measured starting as `plugin:workflow-toolbox:groovy` and answering documentSymbol); `.gradle` stays unmapped because the server reports the Gradle API as unresolvable on an ordinary build script.
...
### Changed
- Java pack: `java` now starts through the plugin's `wt-jdtls` launcher (`node ${CLAUDE_PLUGIN_ROOT}/bin/wt-jdtls.mjs`), which hands Eclipse JDT LS a Java 21+ JVM through its own `--java-executable` (JAVA_HOME when it is 21+, else `java` on PATH, else an installed JDK: SDKMAN, `/usr/lib/jvm`, `/usr/libexec/java_home`, Program Files, `~/.jdks`, or `WT_JDTLS_JDK_DIRS`) and leaves `JAVA_HOME` alone. A session whose `JAVA_HOME` is Java 17 no longer crash-loops the server; with no JDK 21+ installed the launcher refuses in one line naming the requirement.
