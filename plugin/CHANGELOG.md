...
### Added
- SDK runner: a Definition-of-done criterion that the critic blocks on in two consecutive plan rounds is escalated to the run's parent, never to a person. The lifecycle writes `.lane/dod-decision-request.md` (criterion verbatim, plan and critic readings, rounds, deadline) and the runner logs a `decision request:` line; the parent answers with `DECISION DoD <n>: <reading>` in the runner mailbox, which binds the pilot and every later critic brief. Without an answer within 15 minutes the runner applies the narrowest reading that satisfies the card's words, and the pilot report and run summary record which way each term was settled.
