...
### Added
- SDK runner: a Definition-of-done criterion that the critic blocks on in two consecutive plan rounds is escalated to the run's parent, never to a person. The lifecycle writes `.lane/dod-decision-request.md` (an unguessable request id, the criterion verbatim, the plan's reading from its `Card terms: reading chosen` section or, without that section, its Acceptance entry, the critic readings, rounds, deadline) and the runner logs a `decision request:` line; the parent answers with `DECISION <request-id> DoD <n>: <reading>` in the runner mailbox, which binds the pilot and every later critic brief. Only a line quoting this run's request id and appended after the request binds, the latest answer wins, and every other `DECISION` line is ignored and logged. Without an answer within 15 minutes the runner binds one reading by a fixed rule, never a model: the critic now tags each plan-stage blocking finding `[missing]`, `[overbuild]` or `[unverifiable]`; all `[missing]` binds the plan's recorded reading, all `[overbuild]` binds the critic's reading, and anything else binds the card's literal words only. The pilot report and run summary quote the applied reading and the rule that chose it. The plan shape now asks for a mandatory `## Card terms: reading chosen` section; a plan without it is not refused yet, and a decision request records it as absent.
