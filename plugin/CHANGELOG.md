...
- GPT lane executor role table (owner-approved 2026-09-26): code and critic stay on `openai/gpt-6-sol` (code at `high`, critic at `max`), while review moves to `openai/gpt-6-astra` at `medium` so the model reviewing Sol's code is not Sol itself judging its own work; refutation stays on `openai/gpt-6-astra` at `medium`. The forced `xhigh` code variant is now scoped to `openai/gpt-5.6-sol` only — `gpt-6-sol` code resolves to the `high` role base.
