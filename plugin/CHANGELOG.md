...
### Fixed
- Fail SDK role confinement closed: writer Bash now requires an available sandbox and cannot request per-command escape; guard adapter failures explicitly deny; role deny lists merge with caller policy; initialization rejects undeclared tools; and diagnostics/deletion context tools are removed while plugin-controlled fetch, index, search, and LSP services remain documented exceptions.
