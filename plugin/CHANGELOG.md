...
### Fixed
- `wt-second-opinion` gives each Astra call private Codex broker state and stops that exact broker/app-server family on completion, error, signal, or process exit; detached brokers are reached without a before/after PID race that could stop another session's server, and unavailable host cleanup is reported.
