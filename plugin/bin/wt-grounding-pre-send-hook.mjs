...
function main() {
  ...
  const outboundPatterns = process.env.CLAUDE_PLUGIN_OPTION_GROUNDING_OUTBOUND_TOOLS || ''
  if (!outboundClaimTool(payload, outboundPatterns)) return
  ...
  if (refusedKeys.includes(key)) return
  ...
  const emitted = emitGuardNotice({ payload, stdoutJson: { hookSpecificOutput: { hookEventName: 'PreToolUse', permissionDecision: 'deny', permissionDecisionReason: reason } } })
  if (!emitted) {
    journal(payload, 'would-refuse', false, [], checkMode)
    return
  }
