...
Usage:
  node wt-pilot-fidelity.mjs freeze --root <worktree> --out-dir <bundle-dir> \\
    --card <id> --session <id> --base <commit> --head <commit> --file <lane-path> [--other-file <lane-path>]
  node wt-pilot-fidelity.mjs verify --root <worktree> --dir <bundle-dir> [--require-same-tree] [--require-head]

freeze copies the named lane files into a new immutable bundle and records their hashes plus the
current worktree identity. verify refuses changed, missing, extra, symlinked, escaped, malformed,
or signature-mismatched evidence. --require-same-tree and --require-head additionally refuse a
changed tree or HEAD; --require-clean-tree is a deprecated alias for --require-same-tree. Exit 0 means the
requested checks passed; exit 1 means invalid arguments or a refused bundle. It does not establish
...
function parse(argv) {
  ...
  const args = { command, root: null, outDir: null, dir: null, card: null, session: null, base: null, head: null, files: [], otherFiles: [], requireSameTree: false, requireHead: false }
  for (let index = 0; index < rest.length; index += 1) {
    ...
    else if (flag === '--other-file') { const file = value(rest, index, flag); args.files.push(file); args.otherFiles.push(file); index += 1 }
    else if (flag === '--require-same-tree') args.requireSameTree = true
    else if (flag === '--require-clean-tree') { process.stderr.write('deprecated: --require-clean-tree; use --require-same-tree\n'); args.requireSameTree = true }
    else if (flag === '--require-head') args.requireHead = true
    else fail(`unknown argument: ${flag}`)
  }
  if (!args.root) fail('--root is required')
  args.root = resolve(args.root)
  if (command === 'freeze') {
    if (args.requireSameTree || args.requireHead) fail('--require-same-tree and --require-head apply only to verify')
