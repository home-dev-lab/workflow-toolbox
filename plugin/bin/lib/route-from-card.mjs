// Card routing is deliberately small and deterministic: the runner owns it before query().
...
export function deriveRoute(card) {
  ...
  const override = /^\s*Route:\s*(LITE|FULL)\s*$/im.exec(text)?.[1]?.toUpperCase()
  ...
  if (!/definition of done|\bDoD\s*:/i.test(text)) reasons.push('no DoD')
