...
export function createLifecycleStateMachine({
  ...
  function prepareLaneBrief(phase, context, reportPath, snapshotDir = null) {
    ...
    if (!INDEPENDENT_ROLES.has(phase)) {
      ...
      return snapshotDir
        ? { canonical: content, launch: `${content}\nThis brief is the read-only launch snapshot at \`${snapshotDir}\`; if this lane is relaunched, resume from the existing worktree state and preserve the same report and receipt contract. Do not rely on other background processes surviving the lane.\n` }
        : content
    }
    const artifacts = []
