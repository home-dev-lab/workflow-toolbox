...
function main() {
  ...
  const measuredContextKey = payload.transcript_path ? transcriptContextKey(payload.transcript_path) : 'initial'
  const contextKey = measuredContextKey === 'unknown' ? prior.contextKey || 'initial' : measuredContextKey
  ...
  const families = boundedFamilyNames(sources.map((entry) => ({ ...entry, family: entry.layer === 'project' ? `[untrusted project] ${entry.family}` : entry.family }))) || 'none registered; use judgment'
