...
- **Specialist agent types (the `agentType` lever).** Beyond the model tier, a leaf
  ...
  - **Exploit the verify synergy — specialize the producer, not the skeptic.** Route a
    ...
    specialization.
  - **What this routes, and what it does not.** `agentTypes.<role>` / `verifierType` /
    `withAgentDefaults({ agentType })` choose WHICH CLAUDE SUBAGENT TYPE runs that role.
    `codex:codex-rescue` and `workflow-toolbox:opencode-verifier` are still Claude
    subagents — they spend their own turn shelling out to an external CLI. So this never
  ...
  - **Cross-family routing: the composer PROPOSES — never silently applies, never
    ...
    - **Tune the external model's reasoning effort with `OPENCODE_VARIANT`, orthogonal to
      ...
      and version-dependent: as of opencode 1.18.4, `openai/gpt-5.4` accepts `none`…`xhigh`,
       `openai/gpt-5.6-terra`, `openai/gpt-5.6-sol`, `openai/gpt-6-luna`, and `openai/gpt-6-sol` accept `none`…`max` — confirm against your
