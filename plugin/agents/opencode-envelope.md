...
- `OPENCODE_MODEL: <provider/model>` — the default `--model` for tasks that don't override it. Pass it verbatim, including an unknown model; never substitute a known model. If absent, the script defaults to `openai/gpt-6-luna`.
