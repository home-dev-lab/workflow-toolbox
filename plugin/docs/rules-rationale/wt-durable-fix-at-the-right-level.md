...
The operative directives now live in `plugin/rules/wt-durable-fix-at-the-right-level.md`. This file carries the paragraph the shipped `wt-guard-recurrence-hook.mjs` now surfaces mechanically, kept here for provenance.
...
**same guard fired in the same recorded class more than twice in one week → mechanise what it guards, or
