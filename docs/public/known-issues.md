...
| E | **Fable 5 / Mythos 5 are suspended** (US export-control directive, since 2026-06-12) — a verifier (or any agent) pinned to `model: 'fable'` errors at runtime. | `BEST_MODEL` was bumped `'fable'`→`'opus'`, so `adversarialVerification` verifiers default to a callable tier; the toolkit never selects `'fable'` itself. Do **not** hand-override any `model`/`verifierModel` to `'fable'` until the suspension lifts. Revert `BEST_MODEL` when it does. |
