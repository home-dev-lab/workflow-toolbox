...
└── toolkit/                             # pnpm workspace
    ...
    ├── examples/                        # 22 compositions (.workflow.ts) + their tests
...
Plain async functions in workflow definition files that call several patterns in sequence/parallel. The twenty-two shipped examples live here (§6.3). Compositions are **examples and templates, not library API** — copying and editing one is the intended usage.
