...
- **P6 — Add complexity only when it demonstrably improves outcomes.** "Find the simplest solution possible… This might mean not building agentic systems at all." [BEA]. Consequence: every pattern documents *when NOT to use it*, and the toolkit ships the smallest viable surface — seven patterns, fixed.
...
- **Bundling is the bridge.** Workflow scripts cannot `import` anything (sandboxed, no Node APIs), so a TypeScript library reaches the runtime only as a **build artifact**: tsc-strict sources → esbuild IIFE bundle (`--global-name=__dwt`) → concatenated with serialized meta + invocation glue → accepted and executed by the real Workflow tool.
...
│   └── bin/                             #   dwt-debug.mjs (debugger CLI) + dwt-stop-hook.mjs (Stop hook)
...
└── toolkit/                             # pnpm workspace
    ...
    ├── bin/                             # dwt-debug.mjs + dwt-stop-hook.mjs — source-of-truth twins of plugin/bin
...
- **`pnpm dwt:build <entry.ts> [--out-dir <dir>] [--minify]`** — compiles a workflow definition to its artifact (default out-dir `workflows/`, filename = `meta.name`).
- **`pnpm dwt:check <artifact.js>`** — lints an emitted artifact.
- **`pnpm dwt:scaffold <spec.json> [--out-dir <dir>]`** — emits a build-clean `.workflow.ts` skeleton from a `{ meta, steps }` spec.
- **`pnpm dwt:debug [runId|latest]`** — diagnoses a run from its journal (the end-user equivalent is the bundled `node "${CLAUDE_PLUGIN_ROOT}/bin/dwt-debug.mjs"`).
- **`pnpm dwt:report [runId|latest] [--out <dir>]`** — produces a cost + traceability **audit report** for a run from its journal:
...
Every pattern: takes `rt` + a typed options object (prompts as data, items, schemas, caps, model tiering), returns the standard envelope (§7), and assigns its agents to a caller-provided `phase`.
...
- Schemas live next to the pattern that consumes them; patterns accept caller-supplied schemas for their domain payloads and own only their *control* schemas (verdicts, scores, classifications — e.g. the verdict enum `confirmed | partially-confirmed | refuted | unverifiable`).
...
- Keep-unverified-rather-than-drop: a failed verifier never destroys collected evidence; it flags it.
...
- Test-time: every pattern is unit-tested against `FakeRuntime` (scripted, deterministic agent responses; assertions on spawned-agent counts, phases, envelope stats, trail contents, null-handling). Composition tests run all four examples end-to-end against the fake.
...
| Library sprawl (a pattern for everything) | Seven patterns, fixed. New patterns need a demonstrated, repeated real-world use (P6). |
