...
- **The 512 KB cap applies to `scriptPath`** (tool-layer rejection: "exceeds 524288 bytes" — probed directly, the stronger half of this claim) **and to `name` resolution by silent exclusion** — an oversized file in a workflows directory is simply never registered; no error anywhere (observed via registry sweeps only — weaker evidence than the `scriptPath` half). The build CLI therefore pre-checks size, or an oversized workflow would "disappear" with no diagnostic.
...
└── toolkit/                             # pnpm workspace
    ...
    ├── examples/                        # 26 compositions (.workflow.ts) + their tests
...
Every pattern: takes `rt` + a typed options object (prompts as data, items, schemas, caps, model tiering) and returns the standard envelope (§7). All but one assign their agents to a caller-provided `phase` — the exception is `loopUntilDone`, which has no `phase` option (call `rt.phase()` before it); it spawns no agents itself but counts the body's `agent()` calls, made through the `rt` the body receives (including via `rt.parallel`/`rt.pipeline` thunks), into `stats.agentsSpawned`, while its trail stays per-iteration (`trail.length === iterations`) — `agentsSpawned` is counted independently and can coincide with `iterations` (e.g. one agent call per tick), so don't assume the two always diverge.

### L2 — Compositions

Plain async functions in workflow definition files that call several patterns in sequence/parallel. The twenty-six shipped examples live here (§6.3). Compositions are **examples and templates, not library API** — copying and editing one is the intended usage.
...
5. **The dev-workflow family** — `dev-plan`, `dev-implement`, `dev-review-fix`, plus the `dev-full` orchestrator: a TDD feature pipeline with human gates between phases. `dev-plan` (Critique) and `dev-review-fix` (Verify) expose a `verifierType` input for a cross-model verifier, routed across the chain by `dev-full`. Documented in depth in [dev-workflow.md](dev-workflow.md). `dev-ground` is a standalone grounding-first precursor to the family: parallel external-research and internal-code-analysis arms check a card's premises against reality, a PoC canary sub-stage probes what the arms leave unsettled (`refused-by-classifier` / `source-unreachable` are first-class routable outcomes, never errors), then `adversarialVerification` refute-first-checks every premise before a deterministic cancel / reframe / proceed recommendation — the route is always derived in code, never asked of a model.
