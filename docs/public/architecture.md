...
- **P6 — Add complexity only when it demonstrably improves outcomes.** "Find the simplest solution possible… This might mean not building agentic systems at all." [BEA]. Consequence: every pattern documents *when NOT to use it*, and the toolkit ships the smallest viable surface — eight patterns, each one earned against this bar.
...
└── toolkit/                             # pnpm workspace
    ...
    │   ├── patterns/                    # @workflow-toolbox/patterns — the 8 patterns + envelope.ts
    ...
    ├── examples/                        # 13 compositions (.workflow.ts) + their tests
...
Plain async functions in workflow definition files that call several patterns in sequence/parallel. The thirteen shipped examples live here (§6.3). Compositions are **examples and templates, not library API** — copying and editing one is the intended usage.
...
9. **`demo-all-patterns`** — exercises all eight patterns in one workflow; the reference for how they wire together and render in observe-ui.
...
| Library sprawl (a pattern for everything) | Eight patterns. New patterns need a demonstrated, repeated real-world use (P6) — `scoreAndRank` cleared that bar (the cheap-triage-then-aim practice). The count is not frozen; the *bar* is. |
...
| [0006](adr/0006-path-b-hook-observation-removed.md) | Path-B hook observation removed (own-session live observation) |
| [0007](adr/0007-observe-ui-shipped-two-tier.md) | `observe-ui` ships as a two-tier, per-config-dir skill |
