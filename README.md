...
twenty-six runnable example compositions — including a full dev pipeline
...
Twenty-six example compositions live in `toolkit/examples/`; their built artifacts
...
| `wt-observe status` | Report whether a server is running, on which port, serving which sources, and — read from `server.json` — which tree it serves: checkout branch, short commit, and the newest UI bundle. The default when no verb is given. |
| `wt-observe start [--source <dir>]… [--watch] [--enable-launch] [--no-resume] [--allow-branch] [--health-timeout <seconds>]` | Start the server (or adopt one already running on the preferred port). Before starting it prints the served tree — `observe root: branch=<b> head=<sha> bundle=apps/observe-ui/dist/assets/index-*.js` — and records it in `server.json`; an observe root that is not on `main` is REFUSED with the remedy (`git -C <root> checkout main && pnpm ui:build`) unless `--allow-branch` is passed, which prints a loud override line (a served tree must be named, never inferred: two restarts once served a week-old bundle from a forgotten card branch). `--enable-launch` opts into delegated workflow launches. `--no-resume` parks any orphaned run this boot would otherwise auto-resume instead of dispatching it (never discards it — a later boot without the flag resumes it normally). `--health-timeout <seconds>` raises how long `start` waits for the fresh spawn to answer healthy before giving up and reaping it (default 30s, also settable via `WT_OBSERVE_HEALTH_TIMEOUT_MS` in milliseconds, hard ceiling 10 minutes) — useful on a slow machine or when a large resume is expected. |
