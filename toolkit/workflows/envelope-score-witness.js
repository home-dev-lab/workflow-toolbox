export const meta = {
  ...
  "phases": [
    {
      "title": "Triage",
      "detail": "scoreAndRank — one envelope scorer then the rank + cutoff"
    }
  ]
}
var __wt = (() => {
  ...
  // envelope-score-witness.workflow.ts
  var envelope_score_witness_workflow_exports = {};
  __export(envelope_score_witness_workflow_exports, {
    default: () => envelope_score_witness_workflow_default
  });
...
${constraints}`);
    ...
    if (ranked.length >= 2) {
      ...
      if (first !== void 0 && ranked.every((s) => s.score === first.score)) {
        warn(
          rt,
          warnings,
          `${STAGE}: every one of the ${ranked.length} scored items received the IDENTICAL score (${first.score}) \u2014 the ranking is arbitrary and any cutoff below cuts on order, not merit. Either the items really are equivalent, or the scoring prompt is not discriminating.`
        );
      }
    }
  ...
  var HEAD = [`OPENCODE_WORKDIR: ${WORKDIR}`, `OPENCODE_PLUGIN_ROOT: ${PLUGIN_ROOT}`, ""].join("\n");
  var envelope_score_witness_workflow_default = defineWorkflow({
    meta: {
      name: "envelope-score-witness",
      description: "Witness run \u2014 scoreAndRank with the score role on the thin envelope, one item, one dimension, one phase",
      whenToUse: "Only to measure the thin envelope inside scoreAndRank (envelope ladder, matrix row 4). Not a task workflow.",
  ...
  return __toCommonJS(envelope_score_witness_workflow_exports);
})();
