...
${request.renderClaim(request.claim)}`;
  ...
  async function warmVerifierCache(rt, config, effectiveModel, stage, warnings, trail) {
    ...
    trail.push(await runCacheWarmup(rt, warnings, stage("warm"), STAGE4, {
      ...config.phase !== void 0 ? { phase: config.phase } : {},
  ...
  async function adversarialVerification(runtime, options) {
    ...
    const trail = [];
    let agentsSpawned = 0;
    const { salt, warning: stageKeyWarning } = claimStageInstance(rt, STAGE4, config.stageKey);
    if (stageKeyWarning !== void 0) warn(rt, warnings, stageKeyWarning);
    const stage = stageBuilder(STAGE4, salt);
    ...
    if (ranked.length >= 2) {
      ...
      if (first !== void 0 && ranked.every((s) => s.score === first.score)) {
        warn(
          rt,
          warnings,
          `${STAGE8}: every one of the ${ranked.length} scored items received the IDENTICAL score (${first.score}) \u2014 the ranking is arbitrary and any cutoff below cuts on order, not merit. Either the items really are equivalent, or the scoring prompt is not discriminating.`
        );
      }
    }
