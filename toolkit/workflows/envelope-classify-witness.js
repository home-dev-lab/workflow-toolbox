export const meta = {
  ...
  "phases": [
    {
      "title": "Route",
      "detail": "classifyAndAct — one envelope classifier then one envelope action"
    }
  ]
}
var __wt = (() => {
  ...
  // envelope-classify-witness.workflow.ts
  var envelope_classify_witness_workflow_exports = {};
  __export(envelope_classify_witness_workflow_exports, {
    default: () => envelope_classify_witness_workflow_default
  });
...
Never satisfy a constraint with placeholder values ("test", "a"); shorten real content instead of faking it.`;
  ...
  var envelope_classify_witness_workflow_default = defineWorkflow({
    meta: {
      name: "envelope-classify-witness",
      description: "Witness run \u2014 classifyAndAct with the classify role (and the action role) on the thin envelope, one item, one phase",
      whenToUse: "Only to measure the thin envelope inside classifyAndAct (envelope ladder, matrix row 2). Not a task workflow.",
  ...
  return __toCommonJS(envelope_classify_witness_workflow_exports);
})();
