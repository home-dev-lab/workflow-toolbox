export const meta = {
  "name": "envelope-fanout-witness",
  "description": "Witness run — fanOutAndSynthesize with both roles on the thin envelope: one script-generated batch of ten external calls, then one external synthesis over the answer files",
  "whenToUse": "Only to measure the thin envelope inside a real pattern (envelope ladder, rung 2). Not a task workflow.",
  "phases": [
    {
      "title": "Batch",
      "detail": "fanOutAndSynthesize — one envelope task (ten script-generated calls) then one envelope synthesis"
    }
  ]
}
var __wt = (() => {
  ...
  // envelope-fanout-witness.workflow.ts
  var envelope_fanout_witness_workflow_exports = {};
  __export(envelope_fanout_witness_workflow_exports, {
    default: () => envelope_fanout_witness_workflow_default
  });
...
Never satisfy a constraint with placeholder values ("test", "a"); shorten real content instead of faking it.`;
  ...
  // envelope-fanout-witness.workflow.ts
  var WORKDIR = "/home/doublefx/projects/wt-suite/workflow-observatory";
  var SOURCE = `${WORKDIR}/.oc-each-witness.json`;
  var PLUGIN_ROOT = "/home/doublefx/projects/wt-suite/worktrees/card-1844561823-envelope-reap/plugin";
  var envelope_fanout_witness_workflow_default = defineWorkflow({
    meta: {
      name: "envelope-fanout-witness",
      description: "Witness run \u2014 fanOutAndSynthesize with both roles on the thin envelope: one script-generated batch of ten external calls, then one external synthesis over the answer files",
      whenToUse: "Only to measure the thin envelope inside a real pattern (envelope ladder, rung 2). Not a task workflow.",
    ...
    run: async (rt) => {
      const step = await fanOutAndSynthesize(rt, {
        tasks: [SOURCE],
  ...
  return __toCommonJS(envelope_fanout_witness_workflow_exports);
})();
