export const meta = {
  ...
  "description": "One phase, one thin-envelope agent, ten external calls generated BY THE SCRIPT from a file — scripted input, scripted output, the agent only relays. Concurrency is caller-set so the call pattern is observable.",
  ...
  "phases": [
    {
      "title": "Ask",
      "detail": "one envelope agent, N script-generated calls"
    }
  ]
}
var __wt = (() => {
  ...
  // envelope-scripted-io.workflow.ts
  var envelope_scripted_io_workflow_exports = {};
  __export(envelope_scripted_io_workflow_exports, {
    default: () => envelope_scripted_io_workflow_default
  });
...
unreadable channel never fails your task.`;
  ...
  // envelope-scripted-io.workflow.ts
  var envelope_scripted_io_workflow_default = defineWorkflow({
    meta: {
      name: "envelope-scripted-io",
      description: "One phase, one thin-envelope agent, ten external calls generated BY THE SCRIPT from a file \u2014 scripted input, scripted output, the agent only relays. Concurrency is caller-set so the call pattern is observable.",
      whenToUse: "To watch a thin-envelope phase in the observatory with nothing composed by an agent. Not a real analysis: the questions are trivial by design.",
    ...
    run: async (rt, input) => {
      ...
      return {
        relayed: answer,
        passCondition: "a MANIFEST: line above, whose manifest reports answered === the number of source lines and errored === 0"
      };
    }
  });
  return __toCommonJS(envelope_scripted_io_workflow_exports);
})();
