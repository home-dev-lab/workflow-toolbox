...
// `install.mjs --audit-overlap --set rules`, exposed as `pnpm audit:rules-overlap`.
//
// WHY A WRAPPER INSTEAD OF CALLING install.mjs DIRECTLY. `--audit-overlap` requires an
// explicit `--user-dir`, and on a maintainer's machine WITHOUT this project's own local rules
// layout (no `<config dir>/rules`), install.mjs's own `fail()` on a missing dir exits 1
...
//   1  — private rules dir found, audited, at least one non-partial finding (install.mjs
...
const INSTALL_RULES = path.join(HERE, '..', '..', 'plugin', 'skills', 'adopt', 'scripts', 'install.mjs')
