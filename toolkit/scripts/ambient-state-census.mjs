...
export const ambientStateAllowList = new Map([
  ...
  ["packages/build/test/shipped-deep-search.test.ts:real-home-directory:const PRIVATE_HOME_PATH = homedir()", 'leak check derives the private path from the running machine so the path never has to be written into a public file'],
  ...
  ["packages/build/test/second-opinion.test.ts:inherited-env-to-child:const result = spawnSync(process.execPath, [CLI, '--out', f.out, '--route', 'opus'], { encoding: 'utf8', env: { ...process.env, ...f.env } })", 'argument-validation case exits before config, SDK, or global npm resolution'],
