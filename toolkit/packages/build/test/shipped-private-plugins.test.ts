...
describe('shipped private plugins', () => {
  for (const plugin of PLUGINS) {
    ...
    for (const lock of [
      ...
      'V54 5,000 vault entries do not cause per-call vault scans',
      'V55 indexed scrubbing is byte-identical to the old algorithm over generated vault states',
      'V56 an excluded longer value does not hide an eligible prefix',
      'V57 every replacement token for a duplicate value retains eligibility',
      'V58 same-start alternatives preserve clipped merged-span token selection',
      'V59 empty values are never indexed and cannot hang scrubbing',
      'V60 oversized values use bounded matching and never escape in an exception',
      'V61 excluded values allocate no occurrences before eligibility filtering',
      'V62 prefix matching stays within the old per-entry bound for shared prefixes',
      'V63 excluded 4 KiB values complete under a 64 MB heap',
      'V64 duplicate values are filtered once before occurrence scanning',
      'V65 excluded absent prefixes do not scan a 16 million character output',
      'V66 the prefix index retains one entry reference and no per-character nodes',
      'V67 value-map lookup agrees with the old vault scan',
      'V68 long shared prefixes use one forward search per candidate value',
      'V69 impossible-length candidates are rejected before prefix traversal',
