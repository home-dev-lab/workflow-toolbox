...
describe('Claude SDK executor', () => {
  ...
  it('prints a detached pid, loads the guard, writes only the named report, and ends its log with EXIT=0', () => {
    ...
    expect(sdkReceipt.prompt).toContain(`${canonicalPath(join(f.worktree, 'AGENTS.md'))} is the repository's contributor guide; read it before planning or changing code.`)
