...
it('B4 lock: records a contained link and refuses an escaping input link without reading it', () => {
  ...
  const outside = mkdtempSync(join(tmpdir(), 'wt-fidelity-outside-')); roots.push(outside)
  const outsideFile = join(outside, 'outside.log'); writeFileSync(outsideFile, 'outside\n')
  symlinkSync(outsideFile, join(root, '.lane', 'outside.log'))
