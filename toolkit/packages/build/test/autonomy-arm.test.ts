...
describe('--status distinguishes live, expired and absent — and agrees with the watcher', () => {
  ...
  // Conflict with main's deliberate expiry cleanup: retained to record the branch's displaced assertion.
  it.skip('the watcher banner and --status AGREE about the same expired marker at the same instant — the regression this locks', () => {
    const s = scaffold()
    const now = Date.now()
    writeMandate(s.mandatePath, s.sessionId, now - 9 * 60 * 60_000)
