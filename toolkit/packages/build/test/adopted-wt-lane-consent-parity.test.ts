...
`)
  ...
  // The suite-lock executable a lane's WT_SUITE_LOCK_CMD runs, with the library it imports.
...
`)
  ...
  it('hands the adopted lane child an executable WT_SUITE_LOCK_CMD for the installed plugin suite-lock CLI', () => {
    const f = fixture()
    const bin = join(f.root, 'bin')
...
`)
    ...
    const cli = realpathSync(join(f.pluginRoot, 'bin', process.platform === 'win32' ? 'wt-suite-lock.cmd' : 'wt-suite-lock.mjs'))
    expect(readFileSync(seen, 'utf8')).toBe(cli)
    expect(existsSync(cli)).toBe(true)
    const help = process.platform === 'win32'
      ? spawnSync(cli, ['--help'], { encoding: 'utf8', env: f.env, shell: true })
      : runChild('adopted suite-lock CLI help', [cli, '--help'], f.env)
