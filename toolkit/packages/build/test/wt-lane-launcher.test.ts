...
describe.skipIf(process.platform === 'win32')('wt-lane detached launcher (requires POSIX process-group signals; Windows process evidence is transcript-tested)', () => {
  ...
  it.skipIf(!BWRAP_WORKS || !ZSH_WORKS)('runs a lane gate through WT_SUITE_LOCK_CMD inside the real bwrap sandbox (skips without working bwrap or zsh)', () => {
    ...
    const res = run(f, ['--timeout', '30']); expect(res.status, res.stderr).toBe(0)
    waitFor(join(f.dir, '.lane', 'run.log'), 30_000)
    expect(readFileSync(join(f.dir, '.lane', 'run.log'), 'utf8')).toContain('lane sandbox: bwrap')
    const gate = readFileSync(join(f.dir, 'suite-lock-run'), 'utf8')
    expect(gate).toBe('status=0\ngate ran\n')
  }, 60_000)
