...
export function claimSettlement(dir: string, message: WtCommMessage, claim: SettlementClaim): ClaimSettlementResult {
  ...
  try {
    ...
    if (isEexist(e)) return { outcome: 'already-settled' }
    throw e
  }
}
...
export function respondToQuestion(dir: string, question: QuestionMessage, args: RespondToQuestionArgs): RespondToQuestionResult {
  // 1. Read the marker first — already-settled short-circuits WITHOUT writing anything.
  const existing = readSettlement(dir, question.id)
  if (existing.ok) return { outcome: 'already-settled', settlement: existing.settlement }

  // Precondition: the pilot may only decide an option the question actually offered.
  if (!question.payload.options.some((o) => o.id === args.decision)) {
    throw new Error(`wt-comm: decision ${JSON.stringify(args.decision)} is not among question ${JSON.stringify(question.id)}'s options`)
  }

  // 2. Write the decision.response at the deterministic id (get-or-create for its own re-run).
  ...
  // 3. Claim the settlement with mode 'decision'. A crash between step 2 and here leaves
  //    the decision advisory-only; a concurrent default-timeout may already have won —
  //    either way, we re-read the marker below to report the TRUE winner.
  claimSettlement(dir, question, { by: args.by, at: args.at, mode: 'decision', outcome: args.decision })

  const final = readSettlement(dir, question.id)
  if (!final.ok) {
    // Pathological: we (or a racer) just wrote it. Nothing coherent to report.
    return { outcome: 'invalid-claim' }
  }
  return { outcome: final.settlement.mode === 'decision' && final.settlement.outcome === args.decision ? 'settled' : 'already-settled', settlement: final.settlement }
}
