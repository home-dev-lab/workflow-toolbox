...
`to.id` matches your role. A hint is short (its `hint` field is 20–2000 chars of sourced
prose). Unread = no `consumed-<id>.json` marker exists yet for its `id`.

```bash
for f in "$WT_COMM_DIR"/msg-*.json; do
  [ -f "$f" ] || continue
  grep -q '"type":"observer.hint"' "$f" || continue
  grep -q '"id":"'$AGENT_ID'"' "$f" || continue
...
listed as unread forever. First check your `AGENT_ID` contains no double quotes or
backslashes (it should match `^[A-Za-z0-9._-]+$`); if it does not, use a stripped form.

```bash
MARKER="$WT_COMM_DIR/consumed-$ID.json"
CLAIM='{"id":"'$ID'","by":{"role":"agent","id":"'$AGENT_ID'"},"at":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","mode":"read"}'
