...
describe('scanCardCostAgents — explicit-selector scan over a shared flat directory', () => {
  it('matches by NAME, reads identity + usage + activity, and EXCLUDES an unrelated agent in the same dir', () => {
    ...
    expect(a).toMatchObject({ name: 'pilot-x', agentType: 'pilot', model: 'sonnet', description: 'card pilot', transcriptMissing: false })
  ...
  it('a meta.json with NO matching transcript still yields a row (identity intact), with zeroed usage/activity AND transcriptMissing:true', () => {
    const results = scanCardCostAgents(FIXTURES, { names: ['orphan-transcript'] })
    expect(results).toHaveLength(1)
    expect(results[0]).toMatchObject({ agentId: 'cost-c', name: 'orphan-transcript', agentType: 'Explore', transcriptMissing: true })
    expect(results[0]!.usage).toEqual(emptyUsage())
    expect(results[0]!.activity).toEqual(emptyActivity())
  })

  it('TEST-LOCK (cross-family review finding, 2026-07-25): a present, readable transcript is marked transcriptMissing:false even when its usage happens to be zero', () => {
    // agent-cost-b has a real, readable transcript with non-zero usage — sanity that the
    // normal path is false, distinguishing it from the orphan case above which is true.
    const results = scanCardCostAgents(FIXTURES, { names: ['exec-y'] })
    expect(results[0]!.transcriptMissing).toBe(false)
  })
