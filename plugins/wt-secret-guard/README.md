...
`wt-secret-guard` replaces detected secrets in inbound deliveries, pasted prompts, attachments, visible assistant text, and supported tool results with stable tokens. It refuses raw secret-bearing Bash, Write, Edit, NotebookEdit, WebFetch, WebSearch, Agent, Task, and MCP inputs before execution. It can rewrite `op://` 1Password references and `secret:file:/path` references before Bash runs; `secret:env:NAME` is disabled and refused (see [Supported secret references](#supported-secret-references)).
...
MCP tools are covered at their boundary only: a raw secret in an MCP call's input is refused and the result returned to the session is scrubbed, but the secret-file read warning and reference resolution apply to Bash and Read alone. What an MCP server reads or stores on its own side — for example a file a context-indexing server reads itself, or the content it keeps in its own database — never passes through the guard; it is masked only when a later tool result returns it to the session.
...
A reference inside a command substitution (`"$(printf %s secret:file:/path)"`) is **no longer a supported context** (round 11, see "The allow-list" below): it is refused. The literal `op read` form inside one stays accepted.
...
Everything else is refused before the command runs, and the refusal names what was not understood: a reference inside `${...}`, inside backticks or `$'...'`, inside a comment, inside a larger quoted string, preceded by a backslash escape (`\secret:file:/path`), or in an unterminated quote; an `op read` whose reference is not a literal (`"op" read "$REF"`, where the allow-list can read the command; `op read $REF` is out of scope, see the stated limit below) or that carries an undocumented flag or a second reference; an `op` the shell runs as a command, beside a reference, whose verb the guard cannot place (round 16: flags AND redirections may stand between `op` and `read` — `op 2>/dev/null read 'op://…'` is the literal form and is bound; anything else there refuses); `op inject` or `op run` beside a reference; a reference written to a `.tpl` template destination; an unknown form such as `secret:1p:`; a redaction token this session never issued; and a file reference that cannot be read. A refusal never executes the command and never partially expands it.

A `# comment` after a supported reference ends the line rather than opening unfinished syntax: `printf %s secret:file:/path # note` is expanded normally. A reference written *inside* the comment is still refused.
...
Prefer `op://...` or `secret:file:/path` references over pasting raw values (`secret:env:NAME` is disabled, see above). Those references are resolved only when a Bash command runs, so the raw value never passes through the prompt or prompt-history files.
