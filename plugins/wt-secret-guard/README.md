...
### The allow-list — what a command using these forms may contain (round 11, tightened in rounds 12 and 13)
...
- **no backslash-newline line continuation** anywhere outside a quoted heredoc body, and **no carriage return (CR)** anywhere (round 13: `ti\` + newline + `me` is the keyword `time`, and `EOF\r` is a different heredoc delimiter); **shell keywords are recognised in any spelling** — a quoted or escaped `"time"` is refused as a command name too;
