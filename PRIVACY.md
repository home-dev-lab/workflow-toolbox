...
- **External-lane launcher** (`bin/wt-lane.mjs`) — starts a detached local
  ...
  On Linux the CLI runs in a bubblewrap sandbox isolated in filesystem, PID table and
  network: it sees only the worktree, the toolchain and that CLI's own configuration and
  credentials (on private per-run data/cache/state directories), reaches only its configured
  local model endpoint and no other host loopback service, and cannot see `~/.ssh`, `~/.claude`,
  `/run/user/<uid>` or other processes (see known-issues, "External-lane sandbox"). Elsewhere it
  runs with an allow-listed environment only.
